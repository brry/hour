...
\textbf{introduction\\[0.8em] \large start your programming\\journey in a few hours}
\end{textblock*}
\normalsize
\vspace{12em}
Berry Boessenkool, June 2018 - March 2023\\[0.5em]
...
\begin{enumerate}%[<+->]
  ...
  \item Have a look at the \texttt{clim} and \texttt{meta} files. You can do this within Rstudio in the bottomright "Files" panel. ;-) 
\end{enumerate}
\end{exercise}
\end{frame}
...
\begin{enumerate}
  ...
  \item You need to set the arguments \texttt{file}, \texttt{sep}, \texttt{header}\\
        and on R $<$ 4.0.0 (2020-04-24) also \texttt{stringsAsFactors}.\\
...
<<plot_live1, fig.show="hide", size="scriptsize", fig.height=3.0>>=
par(mar=c(3,3,2,1), mgp=c(1.9,0.7,0))
plot(clim$TMK, clim$VPM, xlab="Lufttemperatur", ylab="Dampfdruck", 
     main="Der maximale Dampfdruck ist Temperatur-limitiert
     (Korrelation Tagesmittelwerte Potsdam 2022-2023, DWD)",
     las=1, pch=3, cex=2, col="red", cex.main=0.9)
@
\vspace{0.1em}
<<plot_live2, fig.show="hide", size="scriptsize", fig.height=3.0>>=
...
\includegraphics[width=\textwidth]{figure/plot_live1-1.pdf}
\end{frame}

%------------------------------------------------------------%

\begin{frame}[fragile]{Plotting live demo III}
\includegraphics[width=\textwidth]{figure/plot_live2-1.pdf}
\end{frame}
