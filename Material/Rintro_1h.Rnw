...
\begin{enumerate}%[<+->]
  \item With \rcode{rightclick on} \textbf{\texttt{Raw}} + \rcode{save target as / Download linked file}, download the course 
        \href{https://github.com/brry/hour/blob/master/Material/Rintro.zip}{material} and
...
\vspace{-2em}
<<sol_subset, eval=T, echo=-(1:3), size="scriptsize">>=
options(width=80)
clim <- read.table(file="clim.txt", header=TRUE)
Clim <- head(clim, 20)
clim[1:5, 4]
...
@
\vspace{-0.9em}
\rcode{DF[ ,"name"]} is better understandable for humans and still returns the same if the order of the columns is changed.
But it takes more typing.
\end{frame}
...
<<plot_live, fig.show="hide", size="scriptsize">>=
